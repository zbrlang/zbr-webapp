...
function BotCard({ bot }: { bot: any }) {
  ...
  const { data: meta } = useSWR(`/api/bots/${bot.id}/meta`, fetcher, {
  ...
  const deleteBot = async () => {
    ...
    try {
      ...
      await mutate(
        "/api/bots",
        (bots: any[] = []) => bots.filter((b) => b.id !== bot.id),
        { revalidate: false },
      );
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pb-6 pt-0 relative flex-1">
        <div
          className="absolute -top-10 left-6 h-16 w-16 rounded-2xl bg-surface border-[4px] border-background flex items-center justify-center text-primary text-xl font-black shadow-xl overflow-hidden group-hover:scale-105 transition-transform duration-300"
          style={{
            background: !metaData.avatar
              ? getGradient(bot.name)
              : "transparent",
          }}
        >
          {metaData.avatar ? (
        ...
        <div className="mt-10 mb-6">
          <h3 className="text-xl font-black tracking-tight group-hover:text-primary transition-colors italic">
            {bot.name}
          </h3>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-surface-50 rounded-xl p-3 border border-border-50">
            <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] block mb-0.5">
              Commands
            </span>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-20 space-y-8 md:space-y-0 text-center md:text-left">
            <div>
              ...
              <p className="text-muted-foreground text-lg font-medium">
                Manage and edit your ZBR Discord bot fleet.
              </p>
            </div>
            <Link
              href="/dashboard/new"
            ...
            {Array.isArray(bots) && bots.length > 0 ? (
              ...
              <div className="col-span-full flex flex-col items-center justify-center py-20 text-center bg-surface-20 rounded-[3rem] border border-dashed border-border-50">
                ...
                <p className="text-muted-foreground font-medium max-w-sm mx-auto">
                  Click 'Add New Bot' to get started.
                </p>
              </div>
            )}
          </div>
