  ...
  const handleDeleteCommand = async () => {
    if (selectedCmd) {
      ...
      try {
        ...
        await mutate(`/api/bots/${botId}/commands`, undefined, {
          revalidate: true,
        });
      } catch (e) {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground font-medium">
            Manage your bot's interaction and triggers.
          </p>
        </div>
        <Link
        ...
        {Array.isArray(commands) && commands.length > 0 ? (
          commands.map((cmd: any) => (
            <div
              key={cmd.id}
              onClick={() =>
                router.push(`/dashboard/${botId}/commands/${cmd.id}`)
              }
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground font-mono truncate flex items-center">
                  <span className="bg-primary-10 text-primary px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider border border-primary/20 mr-3">
                    {cmd.type}
                  </span>
                  {cmd.trigger}
                </p>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center bg-surface-20 rounded-3xl border border-dashed border-border-50">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mx-auto">
              Click '+ Create' to add your first command.
            </p>
          </div>
        )}
      </div>
