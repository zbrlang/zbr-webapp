  ...
  return (
    <div className="space-y-10">
      <div>
        ...
        <p className="text-muted-foreground text-sm font-medium">Control how your bot appears to other users on Discord.</p>
      </div>

      <form onSubmit={handleSave} className="space-y-10 max-w-2xl">
        <div className="space-y-4">
          <label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Presence Status</label>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {Array.isArray(statusOptions) ? statusOptions.map((opt) => (
              ...
              >
                <div className={`h-2 w-2 rounded-full ${opt.color} shadow-sm`}></div>
                <span>{opt.label}</span>
              </button>
            )) : null}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Activity Type</label>
          ...
          <div className="space-y-3">
            <label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Activity Name</label>
            <input 
              type="text" 
              value={activityName}
        ...
        <div className="p-6 bg-surface-30 rounded-3xl border border-border-50 space-y-4">
          <h3 className="text-[10px] font-black italic uppercase tracking-widest text-primary/80">Diagnostic Logging</h3>
          <div 
          ...
          >
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-medium">Record command usage and system errors.</p>
            </div>
            <div className={`h-6 w-11 rounded-full relative transition-colors duration-300 ${isLoggingEnabled ? "bg-primary" : "bg-muted"}`}>
              <div className={`absolute top-1 h-4 w-4 bg-accent-foreground rounded-full shadow-sm transition-all duration-300 ${isLoggingEnabled ? "right-1" : "left-1"}`}></div>
            </div>
          </div>
        </div>
