...
`;
  ...
  for (const doc of varsSnapshot.docs) {
    ...
    if (scope === "global") {
      sqliteDb
        .prepare(
          `INSERT INTO global_vars (bot_id, name, value) VALUES (?, ?, ?)`,
        )
        .run(botId, v.name, v.value);
    } else if (scope === "server") {
      sqliteDb
        .prepare(
          `INSERT INTO server_vars (bot_id, guild_id, name, value) VALUES (?, ?, ?, ?)`,
        )
        .run(botId, v.guild_id || "", v.name, v.value);
    } else if (scope === "user") {
      sqliteDb
        .prepare(
          `INSERT INTO user_vars (bot_id, guild_id, user_id, name, value) VALUES (?, ?, ?, ?, ?)`,
        )
        .run(botId, v.guild_id || "", v.user_id || "", v.name, v.value);
    } else if (scope === "channel") {
      sqliteDb
        .prepare(
          `INSERT INTO channel_vars (bot_id, channel_id, name, value) VALUES (?, ?, ?, ?)`,
        )
        .run(botId, v.channel_id || "", v.name, v.value);
    }
  }
