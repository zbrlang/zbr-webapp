...
export default function VariablesTab({
  ...
  const { data: variables = [] } = useSWR<Variable[]>(`/api/bots/${botId}/variables`, fetcher);
  ...
  const handleAddVariable = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await mutate(`/api/bots/${botId}/variables`, undefined, { revalidate: true });
    } catch (e) {
  ...
  const handleEditVariable = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/bots/${botId}/variables/${editingId}`, {
        method: 'PATCH',
  ...
  const deleteVariable = async () => {
    ...
    try {
      const response = await fetch(`/api/bots/${botId}/variables/${selectedVar.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold mb-1 italic uppercase">Variables</h2>
          <p className="text-sm text-muted-foreground font-medium">Store and manage dynamic data for your bot.</p>
        </div>
        <button 
          onClick={() => {
      ...
      {isAdding && (
        <form onSubmit={handleAddVariable} className="bg-surface-50 border border-primary/20 p-6 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Variable Name</label>
              <input 
                type="text" 
                value={newName}
      ...
      {variables && variables.length > 0 ? (
        <div className="grid grid-cols-1 gap-3">
          {variables.map(v => (
            <div key={v.id} className="bg-surface-30 border border-border-50 p-4 rounded-2xl transition-all">
              {editingId === v.id ? (
                <form onSubmit={handleEditVariable} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Value</label>
                      <input 
                        type="text" 
                        value={editValue}
                ...
                >
                  <div className="flex-1">
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">{v.name}</p>
                    <p className="font-bold text-sm font-mono truncate">{v.value || <span className="italic font-normal text-muted-foreground/50">empty</span>}</p>
                  </div>
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center bg-surface-20 rounded-3xl border border-dashed border-border-50">
          ...
          <p className="text-sm text-muted-foreground max-w-xs mx-auto">Click 'Add Variable' to start storing data.</p>
        </div>
      )}
      <ConfirmationModal 
        isOpen={!!selectedVar}
